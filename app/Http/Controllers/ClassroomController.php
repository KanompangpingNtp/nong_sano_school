...
{
    //
    public function ClassRoomHome()
    {
        $postTypes = PostType::all();

        $postTypeId = $postTypes->firstWhere('type_name', 'แนะนำห้องเรียน')->id;

        $postDetails = PostDetail::with('postType', 'pdfs')
            ->where('post_type_id', $postTypeId)
            ->get();

        return view('admin.post.introduce_classroom.introduce_classroom', compact('postDetails', 'postTypes'));
    }
